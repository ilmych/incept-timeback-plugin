...
This reference covers the **staff-dashboard read path** of the Timeback platform — the admin/teacher UI at `https://alpha.timeback.com` that surfaces student activity, goals, mastery, placements, and roster data. **Complementary to** the QTI content-authoring API (`qti.alpha-1edtech.ai`) and the official Cognito-authenticated read APIs (`api.alpha-1edtech.ai` OneRoster / EduBridge / PowerPath — see `auth-cognito.md`, `read-oneroster.md`, `read-edubridge.md`, `read-powerpath.md`, `read-qti.md`).
...
**Auth**: Clerk session cookie (HttpOnly). Cognito `client_credentials` bearer tokens used by QTI / OneRoster / EduBridge / PowerPath do **not** work here. For dashboard `_serverFn` reads you must use a logged-in user's session.
...
This is a **different auth family** from QTI authoring and the official read APIs. Do not send `Authorization: Bearer <cognito>` to `alpha.timeback.com` — those tokens are rejected.

The Clerk session cookie is HttpOnly. Two viable paths for `_serverFn` calls:
...
When the data you need is also exposed on OneRoster / EduBridge / PowerPath / QTI, prefer those Cognito bearer APIs (`api.alpha-1edtech.ai`, `qti.alpha-1edtech.ai`) — see `auth-cognito.md`. That avoids Clerk session expiry. Use this dashboard reference when you need `_serverFn`-only fields (activity heatmaps, goal projections, MAP percentiles, lesson runtime config, etc.).
